 ...
 * 1. 메소드로부터 전달된 데이터를 가공처리 한 뒤 DAO로 전달합니다.
 * 2. DAO로부터 반환받은 결과에 따라서 VIEW로 응답결과를 반환
 */
public class MemberController {
	
	/**
	 * VIEW로 부터 전달받은 문자열 값 다섯 개를 DAO로 전달하는 메소드
	 * 
	...
	public int save(String userId, String userPwd, String userName, String email) {
		...
		// 1) 매개변수 생성자를 호출하여 객체 생성과 동시에 필드값을 대입
	...
	public List<Member> findByKeyword(String keyword) {
		// 결과값이 나중에 어떻게 돌아올까?
		// SELECT -> ResulSet -> Member -> List<Member>
